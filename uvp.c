	...
	for (int i = 1; i < imax + 1; i++) {
		for (int j = 1; j < jmax + 1; j++) {
			...
			if (flag[i][j] & ((1 << 0) | (1 << 3) | (1 << 4))) {
				...
				dut_dx = (1 / dx)
						* ((U[i][j] * (temp[i][j] + temp[i + 1][j]) * 0.5)
								- (U[i - 1][j] * (temp[i - 1][j] + temp[i][j])
										* 0.5))
						+ (alpha / dx)
								* ((fabs(U[i][j])
										* (temp[i][j] - temp[i + 1][j]) * 0.5)
										- (fabs(U[i - 1][j])
												* (temp[i - 1][j] - temp[i][j])
												* 0.5));

				dvt_dy = (1 / dy)
						* ((V[i][j] * (temp[i][j] + temp[i][j + 1]) * 0.5)
								- (V[i][j - 1] * (temp[i][j - 1] + temp[i][j])
										* 0.5))
						+ (alpha / dy)
								* ((fabs(V[i][j])
										* (temp[i][j] - temp[i][j + 1]) * 0.5)
										- (fabs(V[i][j - 1])
												* (temp[i][j - 1] - temp[i][j])
												* 0.5));

				dt2_dx2 = (temp[i + 1][j] - 2 * temp[i][j] + temp[i - 1][j])
						/ (dx * dx);

				dt2_dy2 = (temp[i][j + 1] - 2 * temp[i][j] + temp[i][j - 1])
						/ (dy * dy);
